...
function createTimer(timerData) {
    ...
    startPauseBtn.addEventListener("click", () => {
        if (isRunning) {
            ...
            // Save the start time when the timer is started
            startTime = Date.now();
    ...
    resetBtn.addEventListener("click", () => {
        ...
        worker.postMessage('stop');
        // Clear the start time when the timer is reset
        startTime = undefined;
    });
    ...
    if (timerData) {
        ...
        // Set the initial time of the timer box
        updateTimer(timerDisplay);

        // Check if there is a saved start time
        if (timerData.startTime) {
            // Update the timer display with the elapsed time
            updateTimerElapsed(timerDisplay, timerData.startTime);
        }
    }

    // Save the timer data to local storage
    saveTimerToLocalStorage(timerBox, startTime);
}
...
function saveTimerToLocalStorage(timerBox, startTime) {
...
function updateTimerElapsed(timerDisplay, startTime) {
    if (!startTime) {
        return;
    }
